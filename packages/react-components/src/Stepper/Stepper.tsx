...
import {StepperProps, StepperStepProps} from './StepperProps.types'
...
const Stepper = (props: StepperProps) => {
  ...
  const handleStepClick = (stepIndex: number, step: StepperStepProps) => {
  ...
  const renderStep = (step: StepperStepProps, stepIndex: number) => {
    ...
    return (
      <div
        ...
        {props.showStepNumbers && (
          <div className="recylink-stepper-step-number">{stepIndex + 1}</div>
        )}
        <div className="recylink-stepper-step-content">
          <div className="recylink-stepper-step-label">{step.label}</div>
          {step.description && (
            <div className="recylink-stepper-step-description">{step.description}</div>
          )}
        </div>
      </div>
    )
